...
def transform(path):
    ...
    number = secrets.randbelow(1_000_000_000)
    ...
    oldtree = et.parse(path) # File's loading (beautified version)
    ...
    tree.write("/home/paoloc/Flask_Server/Process" + str(number) + ".xml", encoding='utf-8') # Writing of the new xml file to be ready for the trasformation to the csv one

    xml = et.parse("/home/paoloc/Flask_Server/Process" + str(number) + ".xml") # File's loading
    ...
    for doc in xml.findall("doc"):
        if(doc):
            ...
            if(doc.find("news") != None):
                ...
                with open("/home/paoloc/Flask_Server/Process/process" + str(number) + ".txt", "a", encoding="utf-8") as f:
                    f.writelines(str(news) + "\n")
            else:
                news = None
    
    filename = "/home/paoloc/Flask_Server/Process/process" + str(number) + ".txt"
